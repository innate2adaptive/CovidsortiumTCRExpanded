...
for (f in filenames){
  ...
  data2<-data1[rownames(data1) %in% good,]
  
  cols<-c(paste0(epitope, ".LCMV8.M1"), paste0(epitope, ".LCMV8.M2"), 
          paste0(epitope, ".LCMV8.M3"), paste0(epitope, ".LCMV8.M4"))
  counts_eff<-rowSums(eff[cols])
  good<-names(counts_eff[counts_eff==0]) # I want only the ones that come up later
  data<-data2[rownames(data2) %in% good,]
...
svg("output_figures/Fig5c_sharing_estimate_TCR_LCMV_day40not8.svg")
print(p1)
dev.off()
