...
dir.create(paste0(drive,folder,"HCW_plots/"))
#dir
#I run the script separately for alpha and beta chains; I haev commented out beta chain at the moment
#alpha
...
#####################################################################################################
  ...
  plot(x_jit,y_jit,pch=19,cex=0.5,main=paste(v,time[i],time[j],chain),xlim= c(0,12),ylim=c(0,12),xaxt="n",yaxt="n",ylab="TCRs/million",xlab="TCRs/million")
  ...
  axis(1, at = c(min(x_ss),4,6,8,10),labels = c(0,2^c(4,6,8,10)))
  axis(2, at = c(min(y_ss),4,6,8,10),labels = c(0,2^c(4,6,8,10)),las =2 )
  mx<-mean(x)
  my<-mean(y)
...
#much more straightforward
  imageSave(file=paste0(drive,folder,"HCW_plots/",v,"_",time[i],"_",time[j],"_",chain,".png"))
...
if (chain=="alpha"){
  ...
  save(TCR_change_HCW_a, file = paste0(drive, input_data,"TCR_change_all_alpha.RData"))
    }

if (chain=="beta"){
  TCR_change_HCW_b<-TCR_change_HCW
  save(TCR_change_HCW_b, file = paste0(drive, input_data,"TCR_change_all_beta.RData"))
}
