...
for (f in filenames){
  ...
  zeros<-totals[[epitope]]-dim(data)[1]
  # print(zeros)
  ...
  counts_agg$x<-counts_agg$x/totals[[epitope]]
  print(counts_agg)
  ...
  ######################################################################
  # now repeat analysis only on the ones that are up at day 8
  ######################################################################
...
p1<-ggplot(counts_all_agg) +
  geom_bar(aes(x = logbin, y = as.numeric(x), fill = set), 
           stat = "identity", alpha = 0.7, color = "black", position = position_dodge(preserve = "single"), width = 0.7) +
  scale_fill_manual(values = c(all = "grey", day8 = "deeppink4", day40 = "azure1")) +
...
svg("output_figures/Fig5c_sharing_estimate_TCR_LCMV_allVSday8VSday40not8_freq.svg")
print(p1 + ggtitle("All tetramer-specific vs day 8 vs day 40 (not present at day 8)"))
dev.off()
