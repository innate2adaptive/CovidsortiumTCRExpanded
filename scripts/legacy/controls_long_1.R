...
# now remove expanded
data<-anti_join(data2, exp_AB_wide3, by=c("decombinator_id", "ID"))
print(dim(data))
...
i<-1
for (i in 1:10){
  i_ss<-sample(1:length(data$junction_aa),length(inter))
  print(length(i_ss))
  if (chain == "alpha"){control_a[[i]]<-data$junction_aa[i_ss]}
  if (chain == "beta"){control_b[[i]]<-data$junction_aa[i_ss]}
  }
rm(data)

} # close loop over chains

save(control_a,file="data/output_data/control_alpha_2.RData")
save(control_b,file="data/output_data/control_beta_2.RData")