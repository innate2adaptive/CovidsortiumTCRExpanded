...
imageSave(file=paste0(drive,folder,"test.png"))
...
i_ctrl<-which(exp_AB_wide1$control==T)

#calculate 
#extract controls
exp_AB_ctrl<-exp_AB_wide1[i_ctrl,]
...
boxplot((PCR_total2),add=TRUE,col=NA,axes=FALSE, boxlwd=2,at=c(0:4),border = "green",lwd=1,pars = list(boxwex = 0.4, staplewex = 1, outwex = 1),notch=F)
...
NP<-read.table(file=paste0(drive,folder1,"COVIDsortium_Roche_serology.txt"),check.names=FALSE,sep="\t",header=TRUE,stringsAsFactors = FALSE)
...
imageSave(file=paste0(drive, folder_fig,"NP_time.png"))
...
#SP antibody
#change week to week from PCR
NP<-read.table(file=paste0(drive,folder,"Serology_clinical_09_2020_SP.csv"),sep=",",na.strings="",check.names=FALSE,header=TRUE,stringsAsFactors = FALSE)
