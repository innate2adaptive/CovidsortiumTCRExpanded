...
input_folder<-"Dropbox/Temp (1)/Papers/COVIDsortium/TCR_paper/Michal LCMV data/"

#working folder for collecting output
folder<-"Dropbox/R_temp/21_06_2022/"
#dir.create(paste0(drive,folder))

dir<-dir(paste0(drive,input_folder))
dir
...
k<-1
for (i in c(1,3,5,7)){
data<-read.table(paste0(drive,input_folder,dir[i+8]),sep=",",header = TRUE,row.names = 1)
data_E<-read.table(paste0(drive,input_folder,dir[i]),sep=",",header = TRUE,row.names = 1)
...
# p<-anova[[5]][1]
# anova
# wilcox.test(colsums_E[4:7],colsums)
# 
# 
...
# imageSave(file=paste0(drive,folder,epitope,"naive_eff.png"))
# }
# folder
