...
#NP antibody - Fig S6B
IDs["exp_AB_wide3.ID"]<-as.numeric(as.character(unlist(IDs["exp_AB_wide3.ID"])))
data_merge<-merge(IDs,data,by.x= "exp_AB_wide3.ID", by.y = "Sample_ID")
...
NP_merge1<-merge(data_merge,NP,by.x= "UIN", by.y = "UIN")
NP_merge<-NP_merge1[,14:32]
NP_long <-pivot_longer(NP_merge, cols = 4:19, names_to="week",values_to = "titre")
NP_long$weekfromPCR<-as.numeric(NP_long$week)-as.numeric(NP_long$Week_first)
NP_long$weekfromPCR<-as.numeric(as.character(NP_long$weekfromPCR))
NP_long1<-NP_long[,-4]

#collect together all those samples >12 weeks 
#i_15<-which(NP_long1$weekfromPCR>12)
#NP_long1$weekfromPCR[i_15]<-15
NP_wide<-pivot_wider(NP_long1,names_from = weekfromPCR,values_from="titre", names_sort = T)
NP_wide<-NP_wide[,c(1:3,5:22, 4)]
NP_titre<-NP_wide[,4:22]
#average weeks 12:15
NP_15<-rowMeans(NP_wide[,as.character(11:15)],na.rm=TRUE)
NP_titre_s<-NP_titre[,as.character(-3:5)]
NP_titre_s[,10]<-NP_15
#plot
...
p<- ggplot(NP_titre_s_m) + 
  ...
  scale_x_discrete(labels = c("-3", "-2", "-1", "0", "1", "2", "3", "4", "5", "12-14")) + 
  theme_classic() +
  theme(axis.text=element_text(size=20),
        axis.title=element_text(size=16))
...
early_NP<-rowMeans(NP_titre[,as.character(-3:5)],na.rm=TRUE)
...
NP_long <-pivot_longer(NP_merge, cols = 3:18, names_to="week",values_to = "titre",names_transform = list("titre" = as.numeric))
...
#?pivot_wider
NP_wide<-NP_wide1[,-2]
NP_titre<-NP_wide[,as.character(-3:15)]
NP_titre[3,]
#NP_titre[1,]
#average weeks 12:15
NP_15<-rowMeans(NP_wide[,as.character(10:15)],na.rm=TRUE)
...
early_SP<-rowMeans(NP_titre[,as.character(-3:5)],na.rm=TRUE)
total_SP<-rowMeans(NP_titre,na.rm=TRUE)
# 
